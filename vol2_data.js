...
//（反引号模板字符串不会吞反斜杠，无需双写；这与 vol1_data.js 保持一致）
window.__vol2 = {
  "第六章 平面向量及其应用": [
    {
      title: "6.1 平面向量的概念",
      content: String.raw`<h4>向量的定义</h4><p>既有<strong>大小</strong>又有<strong>方向</strong>的量叫做向量。向量常用有向线段表示，记作$\overrightarrow{AB}$或$\vec{a}$。</p><h4>特殊向量</h4><ul><li><strong>零向量</strong>：长度为$0$的向量，记作$\vec{0}$，方向任意。</li><li><strong>单位向量</strong>：长度等于$1$个单位的向量。</li></ul><h4>向量的关系</h4><ul><li><strong>平行向量（共线向量）</strong>：方向相同或相反的非零向量。规定零向量与任何向量平行。</li><li><strong>相等向量</strong>：长度相等且方向相同的向量。</li><li><strong>相反向量</strong>：长度相等、方向相反的向量，记作$-\vec{a}$。</li></ul>`,
  ...
  "第七章 复数": [
    {
      title: "7.1 复数的概念",
      content: String.raw`<h4>虚数单位</h4><p>规定$i^2=-1$，$i$叫做虚数单位。$i$的幂具有周期性：$i^{4k}=1,\;i^{4k+1}=i,\;i^{4k+2}=-1,\;i^{4k+3}=-i$。</p><h4>复数的定义</h4><p>形如$a+bi$（$a,b\in\mathbb{R}$）的数叫做复数，记作$z=a+bi$。</p><ul><li><strong>实部</strong>：$a=\operatorname{Re}(z)$</li><li><strong>虚部</strong>：$b=\operatorname{Im}(z)$</li></ul><h4>复数的分类</h4><ul><li>$b=0$时，$z$为实数</li><li>$b\neq0$时，$z$为虚数</li><li>$a=0$且$b\neq0$时，$z$为纯虚数</li></ul><h4>共轭复数</h4><p>$z=a+bi$的共轭复数为$\bar{z}=a-bi$。</p><h4>复平面</h4><p>直角坐标系中，$x$轴为实轴，$y$轴为虚轴。复数$z=a+bi$对应点$Z(a,b)$，向量$\overrightarrow{OZ}$。</p><p>复数的模：$|z|=\sqrt{a^2+b^2}$。</p>`,
  ...
  "第八章 立体几何初步": [
    {
      title: "8.1 基本立体图形",
      content: String.raw`<h4>多面体</h4><ul><li><strong>棱柱</strong>：有两个面互相平行，其余各面都是四边形，且相邻两个四边形的公共边互相平行。如三棱柱、四棱柱。</li><li><strong>棱锥</strong>：有一个面是多边形，其余各面都是有一个公共顶点的三角形。</li><li><strong>棱台</strong>：用一个平行于棱锥底面的平面去截棱锥，截面与底面之间的部分。</li></ul><h4>旋转体</h4><ul><li><strong>圆柱</strong>：以矩形的一边为轴旋转一周形成的曲面。</li><li><strong>圆锥</strong>：以直角三角形的一条直角边为轴旋转一周。</li><li><strong>圆台</strong>：以直角梯形的直角腰为轴旋转一周，或用平行于圆锥底面的截面截得。</li><li><strong>球</strong>：以半圆的直径为轴旋转一周，球心$O$，半径$R$。</li></ul>`,
  ...
  "第十章 概率": [
    ...
    {
      title: "10.3 频率与概率",
      content: String.raw`<h4>频率与概率的关系</h4><p>在大量重复试验中，事件$A$发生的频率$f_n(A)=\dfrac{n_A}{n}$会逐渐稳定于某个常数，这个常数就是事件$A$的概率$P(A)$。</p><ul><li><strong>频率</strong>：随试验次数变化的波动值。</li><li><strong>概率</strong>：客观存在的确定值，反映事件发生的可能性大小。</li></ul><h4>大数定律</h4><p>当试验次数$n$足够大时，频率$f_n(A)$趋近于概率$P(A)$。即对任意$\varepsilon>0$，</p><p>$$\lim_{n\to\infty}P\left(\left|\frac{n_A}{n}-P(A)\right|<\varepsilon\right)=1$$</p><h4>用频率估计概率</h4><p>在实际应用中，当无法直接计算概率时，常用大量试验的频率来估计概率（如掷硬币、人口统计等）。试验次数越多，估计越精确。</p>`,
